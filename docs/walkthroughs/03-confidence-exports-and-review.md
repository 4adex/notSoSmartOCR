...
The caller supplies the model and `OPENROUTER_API_KEY`; zero-data retention remains enabled by default. Any retention exception must be explicitly authorized for the data being processed. Raw and refined outputs should be saved separately, including failed requests. Crop recognition can still misread handwriting, associate a note with the wrong row, or omit content. Replacement applies by source ID in a separate rendering view, refreshing affected layout owners while keeping source evidence unchanged. Missing or duplicate crop IDs invalidate the response. Other validation failures are isolated by crop: validated replacements are retained, rejected crops keep their canonical evidence, and mixed results return `partial` with accepted and rejected crop IDs. A result with no accepted crops remains `invalid`; it must not be presented as successful refinement. Selected tables also pass through the existing [TableFormer adapter](../../src/ocr_pipeline/tableformer_structure.py), which predicts structure from the crop independently of Qwen. Disagreement in rows, columns or spans rejects that table replacement. A failed structure prediction is recorded with its error and any invalid predicted grid, and cannot validate a table; it does not discard valid recognition from other crops. Neither agreement nor valid markup proves that cell text is correct. TableFormer can itself miss a column; the pipeline does not trim the Qwen output to force agreement.
